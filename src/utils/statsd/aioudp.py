...
class Protocol(asyncio.DatagramProtocol):
    ...
    def connection_made(self, transport):
        if self._transport is not None:
            ...
            warnings.warn(
                "Reinitializing transport connection from %s to %s",
                old_peer, new_peer
            )
...
async def bind(addr, reuse_port=None):
    ...
    @param addr - For AF_INET or AF_INET6, a tuple with the the host and port
    ...
    @param reuse_port - Tells the kernel to allow this endpoint to be bound to
                    the same port as other existing endpoints are bound to, so
                    long as they all set this flag when being created. This
                    option is not supported on Windows and some UNIX's. If the
                    :py:data:`~socket.SO_REUSEPORT` constant is not defined
